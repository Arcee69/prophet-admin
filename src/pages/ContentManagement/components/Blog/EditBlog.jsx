...
const EditBlog = () => {
    ...
    const formValidationSchema = Yup.object().shape({
        ...
        // imageDoc: Yup.mixed().when("existingImage", {
        //     is: (existingImage) => !existingImage,
    ...
    const submitForm = async (values, actions) => {
        ...
        await api.post(appUrls?.BLOGS_URL + `/${state.id}`, formData)
            .then((res) => {
  ...
  return (
    <div className='flex flex-col gap-5'>
        <p className='font-lato text-[20px] text-GREY-1150'>Edit Blog</p>
        <div className='w-full'>
            <Formik
                initialValues={{
                    title: state.title || "",
                    description: state.body || "",
                    existingImage: state.image || "",
                    imageDoc: null,
                }}
                    ...
                    <Form onSubmit={handleSubmit} className="flex flex-col">
                        <div className='flex flex-col gap-6'>
                            ...
                            <div className='flex flex-col w-full gap-6'>
                                {/* {(values?.imageDoc || values?.existingImage) && (
                                    <div className="pt-0">
                                        <img 
                                            alt="upload" 
                                            width={"300px"} 
                                            height={"100px"} 
                                            src={values?.imageDoc ? URL.createObjectURL(values?.imageDoc) : values?.existingImage} 
                                        />
                                    </div>
                                )} */}
                                ...
                                <label className="flex flex-col xs:w-full  h-56 py-4 px-0  border-2 bg-[#FAFCFF] border-BLUE-_200 border-dashed">
                                    <div className="flex flex-col my-auto items-center">
                                        ...
                                        <div className="text-center font-medium text-sm text-black">
                                            {(values?.imageDoc || values?.existingImage) ? "Replace Thumbnail" : "Click to upload Thumbnails"} <span className='block text-black'>PNG or JPG (max 5mb)</span>
                                        </div>
                                    </div>
                                    <input
                                        type="file"

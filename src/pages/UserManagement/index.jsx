...
const UserManagement = () => {
  ...
  const { users, loading, pagination } = useSelector((state) => state.allUsers)
  ...
  const filteredUsers = users.data?.filter(user =>
    ...
    (status === "" || user.is_blocked === (status === "true")) &&
  ...
  return (
    <div className='flex flex-col gap-[57px]'>
          ...
          <table className='w-full border-collapse '>
            <thead>
              <tr className='bg-NEUTRAL-200'>
                ...
                <th className='p-4 text-left text-sm font-jost font-semibold text-DARK-500'>Subscription</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-GREY-50'}>
                  ...
                  <td className='p-4'>
                    <span className={`${user.is_blocked ? "bg-red-100 text-red-500 " : "bg-GREEN-50 text-GREEN-700 "} text-xs font-medium px-2.5 py-2 rounded-lg`}>{user.is_blocked ? "Blocked" : "Active"}</span>
                  </td>
                  <td className='p-4'>
      ...
      <SideModal isOpen={openDetailsModal} onClose={() => setOpenDetailsModal(false)} width={"md:w-[546px]"}>
        <UserDetails handleClose={() => setOpenDetailsModal(false)}   userData={userData} />
      </SideModal>

      <ModalPop isOpen={openBlockUser}>
        <BlockUser
